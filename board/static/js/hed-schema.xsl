...
<xsl:template match="node[node]">
    ...
    <a href="#x{name}" tag="{name}" description="{description}" role="button" class="list-group-item level-{$level}" data-bs-toggle="collapse" aria-expanded="true" name="schemaNode"><xsl:value-of select="name"/></a>
	<xsl:apply-templates select="attribute">
		<xsl:with-param name="nodeName" select="name"/>
	</xsl:apply-templates>
...
<xsl:template match="unitClassDefinition[unit]">
	<a description="{description}" href="#{translate(translate(name,' ','_'), '0123456789','zowhfvsneit')}" role="button" class="list-group-item" data-bs-toggle="collapse" aria-expanded="true" name="unitClassDef"><xsl:value-of select="name"/></a>
	<xsl:apply-templates select="attribute">
		<xsl:with-param name="nodeName" select="name"/>
	</xsl:apply-templates>
