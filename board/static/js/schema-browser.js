...
function displayResult(xml, useNewFormat, isDeprecated)
{
    ...
    $(".list-group-item:not(.inLibrary,[data-bs-toggle='collapse'])").css('color','#0072B2');
    $(".list-group-item:not(.inLibrary,[data-bs-toggle='collapse'])").css('font-weight','bold'); // make bold leaf node
}
...
function toNode(nodeName) {
    ...
    toLevel(levelNum);
    // findPosition(node);
    $("#schema-browser").animate(
      {
        scrollTop: node.offset().top-100 // 100 is manual ball parking
      },
      500 //speed
    );
